...
7까지는 기본 레이어, 8~31까지는 사용자 지정 레이어로 32비트로 표현된다.

(1) Layer Collision Matrix 에서 체크로 조정할 수 있다.
해제된 레이어끼리는 추올 체크를 하지 않는다.

(2) Culling Mask 를 이용해 게임 뷰에서 보이지 않게 처리할 수 있다.
체크를 해제한 래이어는 실제 존재하지만 보이지 않는다.

(3) 레이 캐스팅 시에 특정 레이어만 캐스팅할 수 있다.
// Player 만 포함
...
if (other.gameObject.layer == LayerMask.NameToLayer(strTag))
if (LayerMask.LayerToName(other.gameObject.layer) == strTag)
...
```
애니메이션 옵션에서 Root Transform 관련 속성의 Bake Into Pose 를 모두 켜주거나, 애니메이션 편집 창에서 Animator.Root 관련 키들을 삭제해주면 된다.
```
<br>
